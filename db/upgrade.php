...
function xmldb_report_sqlqueries_upgrade($oldversion) {
    ...
    if ($oldversion < 2020102800) {
        ...
        $queries = $DB->get_records_select('report_sqlqueries_queries', 'emailto <> ?', [''], 'id', 'id, emailto');
        $total = count($queries);
        ...
        if ($total > 0) {
            ...
            // Now  do the update.
            $progressbar = new progress_bar('report_sqlqueries_emailto_upgrade', 500, true);
            $done = 0;
            foreach ($queries as $query) {
                ...
                $DB->set_field('report_sqlqueries_queries', 'emailto', implode(',', $queryuserids), ['id' => $query->id]);
                $done += 1;
            }
        ...
        // sqlqueries savepoint reached.
        upgrade_plugin_savepoint(true, 2020102800, 'report', 'sqlqueries');
    }
