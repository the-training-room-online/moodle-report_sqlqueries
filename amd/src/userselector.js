 ...
 *
 * @module     report_sqlqueries/userselector
...
export function transport(selector, query, success, failure) {
    Ajax.call([{
        methodname: 'report_sqlqueries_get_users',
        args: {
            query: query,
    ...
    .then((results) => {
        ...
        return Promise.all(results.map((user) => {
            return Templates.render('report_sqlqueries/form-user-selector-suggestion', user)
                .then((html) => {
                    user._label = html;
                    return user;
                });
